...
public class PubClass {
	...
	public void WriteinFile(String str1, String Filename) {
		try {
			...
			BufferedWriter bw = new BufferedWriter(new FileWriter(
					"/data/local/tmp/appium_tfile/" + Filename, true));
			bw.write(str1);
			bw.flush();
			bw.newLine();
			bw.write("\r\n");
	...
	public void pullFile(String filename) {
		byte[] resultDate = driver.pullFile("/data/local/tmp/appium_tfile/"
				+ filename);
	...
	/*
	 * 截屏并保存至本地 screenFile 最好为filepath（图片保存路径） + filename(每张图片的名称）
	 */
	 ...
	 * 
	 * @param cmd
	 *            传入须执行的命令 杀掉app：adb shell am force-stop packagename 启动app：adb
	 *            shell am start -n packagename/第一个启动的Activitity
	 */
	public void extcmd(String cmd) {
		...
		try {
			...
			/*
			 * 支持读取执行结果 BufferedInputStream in = new
			 * BufferedInputStream(process.getInputStream()); byte c[] = new
			 * byte[10]; int n = 0; while ((n = in.read(c)) != -1) { String temp
			 * = new String(c, 0, n); System.out.println(temp); }
			 * 
			 * in.close();
			 */
	...
	public HashMap<String, String> getTestDataInput(String filename) {
		String key = null;
		String[] value = null;
		...
		for (int i = 1; i < result.size(); i++) {
			key = result.get(i).get(1).toString() + "."
					+ result.get(i).get(2).toString();
			// System.out.println(key);
			value = result.get(i).get(3).toString().replace("\"", "")
					.replace(" ", "").split("\\|");
			for (int m = 0; m < value.length; m++) {
				resultmap.put(key + "." + value[m].split("=")[0],
						value[m].split("=")[1]);
			}
		}
